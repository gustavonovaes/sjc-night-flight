...
### v0.0.7 — Correções Multiplayer e Precisão

**Bugs corrigidos:**
- **Precisão 196%:** `playerStats.shotsFired` não contabilizava as balas principais de `_shoot()` — apenas as balas secundárias do Wingman 5G. `shotsHit` contabilizava todas. Fix: `shotsFired += nb.length` após `player.update()` em `game.js`.
- **Lobby aceita 5+ jogadores:** servidor não validava `lobby.players.size >= 4`. Fix: rejeita com erro "Lobby cheio" ao tentar entrar com lobby completo.
- **Lista de jogadores overflow:** altura do box (`lh`) era fixa em 110px (cabe exatamente 4 linhas). Fix: `lh = Math.max(110, 24 + mp.players.size * 21 + 4)` em `drawLobby()` e `mpLobbyHit()`.
- **Unpause volta para solo:** `mp.gameStarted` nunca era definido como `true`, então `mp.connected && mp.gameStarted` era sempre falso ao despausar em multiplayer — retornava `ST.PLAY` em vez de `ST.MULTI`. Fix: `mp.gameStarted = true` em `mpStartGame()`.
- **Nave do jogador local desatualizada na lista:** servidor usa `broadcast` (exclui remetente) para `player_ship`, então `mp.players.get(mp.playerId).planeId` nunca atualizava ao trocar nave. Fix: atualiza `_lrp.planeId` localmente nos três pontos de input (keydown, click, touchstart).
- **Silêncio após desconexão:** `ws.onclose` trocava `gState` para `ST.MENU` sem reiniciar a música. Fix: `stopMusic(); startMenuMusic(selectedDifficulty)` no `onclose`.
- **Inimigos fantasma no cliente:** servidor deletava inimigos fora da tela sem notificar clientes — acumulavam em `enemies[]` indefinidamente. Fix: `broadcastAll(lobby, { type: "enemy_die", id, pts: 0, killedBy: null })` ao sair da tela.
- **Timeout de segundo boss não cancelado:** `setTimeout` do segundo boss em `spawnServerBoss` não era cancelado em `stopLobbyGame`, podendo spawnar inimigos em lobby encerrado. Fix: `lobby.doubleBossTimeout` + `clearTimeout` em `stopLobbyGame`.

**Limitação conhecida (não corrigida):**
- Posição de inimigos diverge entre clientes: cada cliente cria `new Enemy(type, x, y)` com `vx` aleatório independente; servidor move inimigos com `x -= 2` fixo a 30 ticks/s. Requer envio periódico de posições autoritativas pelo servidor.

---

### v0.0.6 — Segurança, Performance e Consistência

- **Segurança:** proteção contra path traversal no servidor estático (`path.resolve` + `import.meta.dir`); IDs de lobby e jogador agora gerados com `crypto.randomBytes()`.
- **Performance:** mensagens de posição (`pos`) enviam buffs e escudos apenas quando alterados (delta update), reduzindo tráfego de rede em partidas longas.
- **Consistência:** multiplicador de HP dos inimigos (`diffCfg.hpMult`) aplicado de forma idêntica no servidor e no cliente.
